...
class SocketServer
{
    ...
    private function newConnection()
    {
        return function ($tag, $keysecret) {
            ...
            if (!empty($keysecret)) {
                $this->connection[$this->connectionIndex]->tag_keysecret = $keysecret;
            } //标记私有连接
    ...
    private function onClose($global)
    {
        return function ($con) use ($global) {
            ...
            if ($con->tag == 'public') {
                ...
                foreach ($this->local_global['public'] as $k => $v) {
                    unset($this->local_global['public'][$k]);
                }
